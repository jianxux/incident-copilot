...
class EvalHarness:
    ...
    def summary(self) -> EvalSummary:
        ...
        if not self.results:
            return EvalSummary(
                total_incidents=0, passed=0, failed=0,
                avg_score=0, avg_latency_ms=0, avg_tokens=0,
        ...
        avg_score = sum(r.rubric.weighted_score for r in self.results) / len(self.results)
        ...
        for r in self.results:
            if r.scenario_type not in by_scenario:
                by_scenario[r.scenario_type] = {"count": 0, "passed": 0, "total_score": 0}
...
async def run_quick_eval(copilot=None, count: int = 10):
    ...
    print(f"Passed: {summary.passed} ({summary.passed / summary.total_incidents * 100:.1f}%)")
    ...
    for scenario, data in summary.by_scenario.items():
        print(f"  {scenario}: {data['passed']}/{data['count']} passed, avg={data['avg_score']:.2f}")
    print("\nFailure Severities:")
