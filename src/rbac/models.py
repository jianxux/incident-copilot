...
class Role(BaseModel):
    ...
    inherits_from: list[UUID] = Field(
        default_factory=list, description="Roles this role inherits from"
    )
...
SYSTEM_ROLES = {
    ...
    "responder": Role(
        ...
        permissions=[
            ...
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.INCIDENT, action=Action.ACKNOWLEDGE
                )
            ),
            RolePermission(
                permission=Permission(
    ...
    "team_lead": Role(
        ...
        permissions=[
            ...
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.ESCALATION_POLICY, action=Action.CREATE
                )
            ),
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.ESCALATION_POLICY, action=Action.READ
                )
            ),
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.ESCALATION_POLICY, action=Action.UPDATE
                )
            ),
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.ESCALATION_POLICY, action=Action.DELETE
                )
            ),
            # Maintenance windows
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.MAINTENANCE_WINDOW, action=Action.CREATE
                )
            ),
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.MAINTENANCE_WINDOW, action=Action.READ
                )
            ),
            RolePermission(
                permission=Permission(
                    resource_type=ResourceType.MAINTENANCE_WINDOW, action=Action.UPDATE
                )
            ),
            # Reports
            RolePermission(
