...
from .models import (ChartType, DashboardCreate, DashboardLayout, GridPosition,
                     WidgetConfig, WidgetCreate, WidgetDataSource, WidgetType)


def _widget(title: str, wtype: WidgetType, metric: str, x: int, y: int, w: int = 3, h: int = 2, **kw) -> WidgetCreate:
    return WidgetCreate(title=title, config=WidgetConfig(widget_type=wtype, data_source=WidgetDataSource(metric=metric), **kw),
...
def _chart(title: str, metric: str, ct: ChartType, x: int, y: int, w: int = 6, h: int = 3, **kw) -> WidgetCreate:
    return _widget(title, WidgetType.CHART, metric, x, y, w, h, chart_type=ct, **kw)


def _list(title: str, metric: str, x: int, y: int, w: int = 6, h: int = 4, cols: list[str] | None = None) -> WidgetCreate:
    return _widget(title, WidgetType.LIST, metric, x, y, w, h, columns=cols or ["title", "severity", "status"])


def _timeline(title: str, metric: str, x: int, y: int, w: int = 12, h: int = 3, grouped: bool = False) -> WidgetCreate:
    return _widget(title, WidgetType.TIMELINE, metric, x, y, w, h, group_by_service=grouped)


def _heatmap(title: str, metric: str, x: int, y: int, w: int = 6, h: int = 4, scheme: str = "severity") -> WidgetCreate:
    return _widget(title, WidgetType.HEATMAP, metric, x, y, w, h, color_scheme=scheme)


EXECUTIVE_DASHBOARD = DashboardCreate(
    name="Executive Overview", description="High-level metrics for leadership", is_default=True, role="executive",
    tags=["default", "executive"], layout=DashboardLayout(row_height=100),
    widgets=[
...
ONCALL_DASHBOARD = DashboardCreate(
    name="On-Call Dashboard", description="Real-time view for on-call engineers", is_default=True, role="oncall",
    tags=["default", "oncall"], layout=DashboardLayout(),
    widgets=[
        _counter("Open", "incidents.open", 0, 0),
        _counter("Unacknowledged", "incidents.unacked", 3, 0, threshold_warning=3, threshold_critical=5),
        _counter("Assigned to Me", "incidents.mine", 6, 0),
        _counter("Avg Response", "incidents.response", 9, 0),
        _list("Active Incidents", "incidents.active", 0, 2, 6, 5, ["severity", "title", "service", "duration"]),
...
SRE_DASHBOARD = DashboardCreate(
    name="SRE Dashboard", description="Detailed metrics for SRE team", is_default=True, role="sre",
    tags=["default", "sre"], layout=DashboardLayout(),
    widgets=[
...
MANAGER_DASHBOARD = DashboardCreate(
    name="Team Manager Dashboard", description="Team performance metrics", is_default=True, role="manager",
    tags=["default", "manager"], layout=DashboardLayout(row_height=90),
    widgets=[
        ...
        _list("SLA At Risk", "incidents.sla_risk", 6, 5, 6, 4, ["title", "sla_remaining", "assignee"]),
    ])

DEFAULT_DASHBOARDS = {"executive": EXECUTIVE_DASHBOARD, "oncall": ONCALL_DASHBOARD,
                      "sre": SRE_DASHBOARD, "manager": MANAGER_DASHBOARD}
