...
from .models import Dashboard, DashboardCreate, DashboardSummary, DashboardUpdate, ShareConfig, ShareScope, Widget, WidgetCreate, WidgetUpdate
...
async def create_dashboard(data: DashboardCreate, uid: Annotated[UUID, Depends(get_user_id)],
                           svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def list_dashboards(uid: Annotated[UUID, Depends(get_user_id)], svc: Annotated[DashboardService, Depends(get_service)],
                          tags: Annotated[list[str] | None, Query()] = None, role: str | None = None) -> list[DashboardSummary]:
...
async def create_from_default(role: str, uid: Annotated[UUID, Depends(get_user_id)],
                              svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
...
async def get_public_dashboard(token: str, svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def get_dashboard(did: UUID, uid: Annotated[UUID, Depends(get_user_id)],
                        svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def update_dashboard(did: UUID, data: DashboardUpdate, uid: Annotated[UUID, Depends(get_user_id)],
                           svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def delete_dashboard(did: UUID, uid: Annotated[UUID, Depends(get_user_id)],
                           svc: Annotated[DashboardService, Depends(get_service)]) -> None:
    try:
...
async def clone_dashboard(did: UUID, req: CloneRequest, uid: Annotated[UUID, Depends(get_user_id)],
                          svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def update_sharing(did: UUID, req: ShareRequest, uid: Annotated[UUID, Depends(get_user_id)],
                         svc: Annotated[DashboardService, Depends(get_service)]) -> ShareConfig:
    try:
        return await svc.update_sharing(did, uid, req.scope, req.team_ids, req.user_ids, req.allow_edit, req.expires_at)
    except DashboardNotFoundError as e:
        _err(e)
    except PermissionDeniedError as e:
        _err(e, 403)
...
async def add_widget(did: UUID, data: WidgetCreate, uid: Annotated[UUID, Depends(get_user_id)],
                     svc: Annotated[DashboardService, Depends(get_service)]) -> Widget:
    try:
...
async def update_widget(did: UUID, wid: UUID, data: WidgetUpdate, uid: Annotated[UUID, Depends(get_user_id)],
                        svc: Annotated[DashboardService, Depends(get_service)]) -> Widget:
    try:
...
async def delete_widget(did: UUID, wid: UUID, uid: Annotated[UUID, Depends(get_user_id)],
                        svc: Annotated[DashboardService, Depends(get_service)]) -> None:
    try:
...
async def reorder_widgets(did: UUID, data: WidgetPositions, uid: Annotated[UUID, Depends(get_user_id)],
                          svc: Annotated[DashboardService, Depends(get_service)]) -> Dashboard:
    try:
...
async def get_widget_data(did: UUID, wid: UUID, uid: Annotated[UUID, Depends(get_user_id)],
                          svc: Annotated[DashboardService, Depends(get_service)]) -> dict[str, Any]:
    try:
...
async def dashboard_ws(ws: WebSocket, did: UUID, svc: Annotated[DashboardService, Depends(get_service)]) -> None:
    try:
