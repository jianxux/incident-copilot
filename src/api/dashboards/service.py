...
from .models import (Dashboard, DashboardCreate, DashboardSummary, DashboardUpdate,
                     ShareConfig, ShareScope, Widget, WidgetCreate, WidgetUpdate)
from .widgets import validate_widget_config


class DashboardNotFoundError(Exception): pass
class WidgetNotFoundError(Exception): pass
...
class DashboardService:
    ...
    async def create_dashboard(self, owner_id: UUID, data: DashboardCreate) -> Dashboard:
        ...
        for wd in data.widgets:
            ...
            widgets.append(Widget(id=uuid4(), dashboard_id=did, created_at=now, updated_at=now, **wd.model_dump()))
        
        dashboard = Dashboard(id=did, owner_id=owner_id, name=data.name, description=data.description,
                              layout=data.layout, tags=data.tags, is_default=data.is_default,
                              role=data.role, widgets=widgets, created_at=now, updated_at=now)
    ...
    async def list_dashboards(self, user_id: UUID, team_ids: list[UUID] | None = None,
                              tags: list[str] | None = None, role: str | None = None) -> list[DashboardSummary]:
        results = []
        for d in self._dashboards.values():
            ...
            if role and d.role != role:
                continue
            results.append(DashboardSummary(id=d.id, name=d.name, description=d.description, owner_id=d.owner_id,
                                            widget_count=len(d.widgets), share_scope=d.share_config.scope,
    ...
    async def update_widget(self, did: UUID, wid: UUID, user_id: UUID, data: WidgetUpdate) -> Widget:
        d = await self.get_dashboard(did, user_id)
        if not await self._can_edit(d, user_id):
            raise PermissionDeniedError("No edit access")
    ...
    async def reorder_widgets(self, did: UUID, user_id: UUID, positions: dict[UUID, dict[str, int]]) -> Dashboard:
        d = await self.get_dashboard(did, user_id)
        if not await self._can_edit(d, user_id):
            raise PermissionDeniedError("No edit access")
        for w in d.widgets:
    ...
    async def clone_dashboard(self, did: UUID, user_id: UUID, new_name: str | None = None) -> Dashboard:
        src = await self.get_dashboard(did, user_id)
        nid, now = uuid4(), datetime.utcnow()
        widgets = [Widget(id=uuid4(), dashboard_id=nid, title=w.title, description=w.description,
                          config=w.config.model_copy(deep=True), position=w.position.model_copy(),
                          refresh_interval_seconds=w.refresh_interval_seconds, date_range=w.date_range.model_copy(),
                          created_at=now, updated_at=now) for w in src.widgets]
        clone = Dashboard(id=nid, owner_id=user_id, name=new_name or f"{src.name} (Copy)",
                          description=src.description, layout=src.layout.model_copy(), tags=src.tags.copy(),
                          widgets=widgets, created_at=now, updated_at=now, cloned_from=src.id)
        self._dashboards[nid] = clone
        return clone
    
    async def update_sharing(self, did: UUID, user_id: UUID, scope: ShareScope, team_ids: list[UUID] | None = None,
                             user_ids: list[UUID] | None = None, allow_edit: bool = False,
                             expires_at: datetime | None = None) -> ShareConfig:
        d = await self.get_dashboard(did, user_id)
        if d.owner_id != user_id:
        ...
        if scope == ShareScope.PUBLIC and not token:
            ...
            token = None
        d.share_config = ShareConfig(scope=scope, team_ids=team_ids or [], shared_with_users=user_ids or [],
                                     public_token=token, expires_at=expires_at, allow_edit=allow_edit)
        d.updated_at = datetime.utcnow()
    ...
    async def _can_edit(self, d: Dashboard, uid: UUID) -> bool:
        return d.owner_id == uid or (d.share_config.allow_edit and uid in d.share_config.shared_with_users)
