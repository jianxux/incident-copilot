...
class WidgetRenderer(ABC):
    @abstractmethod
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]: pass
    
    def validate(self, cfg: WidgetConfig) -> list[str]: return []
...
class CounterRenderer(WidgetRenderer):
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]:
        ...
        return {"value": 0, "previous": 0, "trend_percent": 0.0, "status": "normal",
                "time_range": {"start": s.isoformat(), "end": e.isoformat()}}
    
    def validate(self, cfg: WidgetConfig) -> list[str]:
        if cfg.threshold_warning and cfg.threshold_critical and cfg.threshold_warning >= cfg.threshold_critical:
...
class ChartRenderer(WidgetRenderer):
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]:
        s, e = resolve_date_range(dr)
        return {"chart_type": (cfg.chart_type or ChartType.LINE).value, "series": [], "labels": [],
                "time_range": {"start": s.isoformat(), "end": e.isoformat()}, "stacked": cfg.stacked}
...
class ListRenderer(WidgetRenderer):
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]:
        s, e = resolve_date_range(dr)
        return {"items": [], "total": 0, "page_size": cfg.page_size,
...
class TimelineRenderer(WidgetRenderer):
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]:
        s, e = resolve_date_range(dr)
        return {"events": [], "grouped": cfg.group_by_service,
                "time_range": {"start": s.isoformat(), "end": e.isoformat()}}
...
class HeatmapRenderer(WidgetRenderer):
    async def fetch_data(self, cfg: WidgetConfig, dr: DateRange) -> dict[str, Any]:
        s, e = resolve_date_range(dr)
        return {"cells": [], "x_labels": [], "y_labels": [], "scheme": cfg.color_scheme,
                "time_range": {"start": s.isoformat(), "end": e.isoformat()}}
...
RENDERERS: dict[WidgetType, type[WidgetRenderer]] = {
    WidgetType.COUNTER: CounterRenderer, WidgetType.CHART: ChartRenderer,
    WidgetType.LIST: ListRenderer, WidgetType.TIMELINE: TimelineRenderer,
...
async def fetch_widget_data(w: Widget) -> dict[str, Any]:
    ...
    return {"widget_id": str(w.id), "type": w.config.widget_type.value, "title": w.title,
            "data": data, "fetched_at": datetime.utcnow().isoformat(), "refresh": w.refresh_interval_seconds}
