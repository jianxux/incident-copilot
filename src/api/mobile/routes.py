...
from .models import (
    BiometricAuthRequest, BulkActionRequest, BulkActionResponse, CommentCreate,
    CommentMinimal, DashboardSummary, DeviceRegistration, DeviceRegistrationResponse,
    IncidentCompact, IncidentFull, IncidentListResponse, IncidentMinimal, IncidentStatus,
    NotificationPreferences, PaginationMeta, QuickActionRequest, QuickActionResponse,
    QuickActionType, Severity, SeverityCount, SyncCheckRequest, SyncCheckResponse,
...
class MockIncidentService:
    _incidents = [
        {"id": f"inc_{i}", "title": f"Database timeout #{i}", "severity": ["critical", "high", "medium", "low"][i % 4],
         "status": ["open", "acknowledged"][i % 2], "service": "api", "assignee": "user_123" if i % 2 == 0 else None,
         "created_at": "2024-01-15T10:00:00Z", "updated_at": "2024-01-15T10:30:00Z", "comment_count": i}
        for i in range(20)
    ]

    async def list_incidents(self, cursor: str | None, limit: int, status_filter: list | None,
                            severity_filter: list | None, assignee: str | None):
        start = int(cursor) if cursor else 0
        filtered = [i for i in self._incidents
        ...
        return items, str(start + limit) if start + limit < len(filtered) else None, start + limit < len(filtered)
    ...
    async def add_comment(self, incident_id: str, user_id: str, text: str, internal: bool):
        return {"id": f"c_{int(time.time())}", "text": text, "author": user_id, "ts": int(time.time()), "is_internal": internal}

    async def get_dashboard(self, user_id: str):
        return {"open_count": 8, "ack_count": 3, "my_incidents": 2,
                "by_severity": {"critical": 2, "high": 3, "medium": 2, "low": 1}, "mttr_hours": 2.5}
...
def to_minimal(inc: dict) -> IncidentMinimal:
    return IncidentMinimal(id=inc["id"], title=inc["title"][:100], severity=Severity(inc["severity"]),
                          status=IncidentStatus(inc["status"]), ts=int(time.time()))


def to_compact(inc: dict) -> IncidentCompact:
    return IncidentCompact(id=inc["id"], title=inc["title"][:100], severity=Severity(inc["severity"]),
                          status=IncidentStatus(inc["status"]), ts=int(time.time()), service=inc.get("service"),
                          assignee=inc.get("assignee"), comment_count=inc.get("comment_count", 0))


def to_full(inc: dict) -> IncidentFull:
    return IncidentFull(id=inc["id"], title=inc["title"], severity=Severity(inc["severity"]),
                       status=IncidentStatus(inc["status"]), ts=int(time.time()), service=inc.get("service"),
...
async def list_incidents(
    ...
    incidents, next_cursor, has_more = await svc.list_incidents(
        cursor, limit, [s.value for s in status] if status else None,
        [s.value for s in severity] if severity else None, user["id"] if mine else None)
    
    etag = compute_etag(incidents)
    if if_none_match == etag:
        return Response(status_code=304)
    
    items = [to_compact(i) for i in incidents] if expand == "compact" else [to_minimal(i) for i in incidents]
    response.headers.update({"ETag": etag, "Cache-Control": "private, max-age=30"})
    return IncidentListResponse(items=items, meta=PaginationMeta(cursor=next_cursor, has_more=has_more), etag=etag)
...
async def quick_action(req: QuickActionRequest, user: dict = Depends(get_current_user), svc=Depends(get_service)):
    ...
    return QuickActionResponse(success=True, incident_id=req.incident_id, new_status=new_status, message=msg, ts=int(time.time()))


@router.post("/actions/bulk", response_model=BulkActionResponse)
async def bulk_action(req: BulkActionRequest, user: dict = Depends(get_current_user), svc=Depends(get_service)):
...
async def add_comment(incident_id: str, comment: CommentCreate, user: dict = Depends(get_current_user), svc=Depends(get_service)):
    ...
    return CommentMinimal(id=r["id"], text=r["text"], author=user["name"], ts=r["ts"], is_internal=r["is_internal"])


@router.get("/dashboard", response_model=DashboardSummary)
async def get_dashboard(response: Response, user: dict = Depends(get_current_user), svc=Depends(get_service)):
    """Mobile dashboard summary."""
    d = await svc.get_dashboard(user["id"])
    response.headers["Cache-Control"] = "private, max-age=60"
    return DashboardSummary(open_count=d["open_count"], ack_count=d["ack_count"], my_incidents=d["my_incidents"],
                           by_severity=SeverityCount(**d["by_severity"]), mttr_hours=d.get("mttr_hours"), ts=int(time.time()))
...
async def register_device(reg: DeviceRegistration, user: dict = Depends(get_current_user)):
    ...
    return DeviceRegistrationResponse(success=True, device_id=reg.device_id, expires_at=int(time.time()) + 86400 * 30)
...
async def refresh_token(req: TokenRefreshRequest):
    ...
    return TokenRefreshResponse(access_token=f"at_{int(time.time())}", refresh_token=f"rt_{int(time.time())}",
                               expires_in=3600, biometric_hint=True)
...
async def biometric_auth(req: BiometricAuthRequest):
    ...
    return TokenRefreshResponse(access_token=f"bio_{int(time.time())}", refresh_token=f"brt_{int(time.time())}",
                               expires_in=3600, biometric_hint=True)
...
async def check_sync(req: SyncCheckRequest):
    ...
    return SyncCheckResponse(has_updates=has_updates, updated_ids=["inc_0"] if has_updates else [],
                            deleted_ids=[], server_ts=int(time.time()))
...
async def get_batch(ids: Annotated[list[str], Query()], svc=Depends(get_service)):
    ...
    return {"items": [to_full(i) for iid in ids if (i := await svc.get_incident(iid))], "ts": int(time.time())}
