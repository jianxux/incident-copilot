...
from ..plugins.models import Plugin, PluginCreateRequest, PluginEvent, PluginStatus, PluginTestRequest, PluginTestResult, PluginType, PluginUpdateRequest
...
async def list_plugins(type: PluginType | None = None, status_filter: PluginStatus | None = Query(None, alias="status"), event: PluginEvent | None = None) -> list[Plugin]:
...
async def test_plugin(plugin_id: str, request: PluginTestRequest) -> PluginTestResult:
    try:
        sample = request.sample_data or {"incident_id": "INC-12345", "title": "Test incident", "severity": "high", "service_name": "test-service"}
...
async def enable_plugin(plugin_id: str) -> Plugin:
    try:
        return await get_registry().update(plugin_id, PluginUpdateRequest(status=PluginStatus.ACTIVE))
    except PluginNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
async def disable_plugin(plugin_id: str) -> Plugin:
    try:
        return await get_registry().update(plugin_id, PluginUpdateRequest(status=PluginStatus.DISABLED))
    except PluginNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
