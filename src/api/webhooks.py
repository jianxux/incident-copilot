...
async def pagerduty_webhook(request: Request, background_tasks: BackgroundTasks, x_pagerduty_signature: str | None = Header(None, alias="X-PagerDuty-Signature")):
    """Receive PagerDuty v3 webhook events with alert correlation."""
    settings = get_settings()
    body = await request.body()
    pd_adapter = PagerDutyAdapter(settings)
    if x_pagerduty_signature and not pd_adapter.verify_webhook_signature(body, x_pagerduty_signature):
    ...
    logger.info("pagerduty_incident_received", incident_id=incident.incident_id, service=incident.service_name, severity=incident.severity.value)
    background_tasks.add_task(process_pagerduty_incident_background, incident, settings)
    return {"status": "accepted", "incident_id": incident.incident_id, "service": incident.service_name}

@router.post("/opsgenie")
async def opsgenie_webhook(request: Request, background_tasks: BackgroundTasks, x_opsgenie_signature: str | None = Header(None, alias="X-OpsGenie-Signature")):
    ...
    logger.info("opsgenie_alert_received", alert_id=alert.alert_id, service=alert.service_name, severity=alert.severity.value)
    background_tasks.add_task(process_opsgenie_alert_background, alert, settings)
    return {"status": "accepted", "alert_id": alert.alert_id, "service": alert.service_name}
    ...
    try:
        if getattr(settings, "correlation_enabled", True):
            ...
            if not result.should_notify:
                logger.info("pagerduty_notification_suppressed", incident_id=incident.incident_id, group_id=result.group.group_id if result.group else None, reason=result.suppression_reason)
                return
            logger.info("pagerduty_notification_allowed", incident_id=incident.incident_id, correlated=result.correlated, new_group=result.new_group)
        orchestrator = ContextOrchestrator(settings)
        await orchestrator.process_incident(incident)
    except Exception as e:
        logger.error("pagerduty_background_processing_failed", incident_id=incident.incident_id, error=str(e))
...
async def process_opsgenie_alert_background(alert, settings):
    ...
    try:
        if getattr(settings, "correlation_enabled", True):
            ...
            if not result.should_notify:
                logger.info("opsgenie_notification_suppressed", alert_id=alert.alert_id, group_id=result.group.group_id if result.group else None, reason=result.suppression_reason)
                return
            logger.info("opsgenie_notification_allowed", alert_id=alert.alert_id, correlated=result.correlated, new_group=result.new_group)
        from ..models import PagerDutyIncident
        pd_incident = PagerDutyIncident(incident_id=alert.alert_id, title=alert.title, description=alert.description, severity=alert.severity, service_name=alert.service_name, triggered_at=alert.triggered_at, html_url=alert.url)
        orchestrator = ContextOrchestrator(settings)
        await orchestrator.process_incident(pd_incident)
    except Exception as e:
        logger.error("opsgenie_background_processing_failed", alert_id=alert.alert_id, error=str(e))
