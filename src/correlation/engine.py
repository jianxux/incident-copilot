...
from .models import AlertGroup, AlertGroupStatus, CorrelationResult, CorrelationRule, IncomingAlert
...
class CorrelationEngine:
    def __init__(self, settings: Settings, store: CorrelationStore | None = None, rule_manager: RuleManager | None = None):
        self.settings = settings
        self.store = store or CorrelationStore(redis_url=settings.redis_url, group_ttl_seconds=getattr(settings, "correlation_group_ttl", 86400))
    ...
    def normalize_pagerduty(self, incident: PagerDutyIncident) -> IncomingAlert:
        return IncomingAlert(alert_id=incident.incident_id, source="pagerduty", title=incident.title, description=incident.description,
                            service=incident.service_name, severity=incident.severity.value, triggered_at=incident.triggered_at, url=incident.html_url,
                            tags=[f"service:{incident.service_name}", f"severity:{incident.severity.value}"],
                            extra={"incident_number": incident.incident_number, "service_id": incident.service_id, "assigned_to": incident.assigned_to})
    ...
    def normalize_opsgenie(self, alert: OpsgenieAlert) -> IncomingAlert:
        ...
        return IncomingAlert(alert_id=alert.alert_id, source="opsgenie", title=alert.title, description=alert.description,
                            service=alert.service_name, severity=alert.severity.value, tags=tags, triggered_at=alert.triggered_at,
                            url=alert.url, extra={"tiny_id": alert.tiny_id, "source": alert.source})
    ...
    async def correlate(self, alert: IncomingAlert) -> CorrelationResult:
        ...
        return await self._add_to_group(alert, existing_group, rule) if existing_group else await self._create_group(alert, rule)
    ...
    async def _add_to_group(self, alert: IncomingAlert, group: AlertGroup, rule: CorrelationRule) -> CorrelationResult:
        ...
        if rule.suppress_duplicates:
            if group.notification_sent:
                if rule.re_notify_after_seconds > 0 and (datetime.utcnow() - (group.first_alert_at or group.created_at)).total_seconds() >= rule.re_notify_after_seconds:
                    should_notify = True
                else:
        ...
        logger.info("alert_correlated", alert_id=alert.alert_id, group_id=group.group_id, should_notify=should_notify)
        return CorrelationResult(alert=alert, correlated=True, group=group, new_group=False, rule_matched=rule, should_notify=should_notify, suppression_reason=suppression_reason)
    ...
    async def _create_group(self, alert: IncomingAlert, rule: CorrelationRule) -> CorrelationResult:
        ...
        window_expires = datetime.utcnow() + timedelta(seconds=rule.time_window_seconds) if rule.time_window_seconds > 0 else None
        group = AlertGroup(group_id=f"grp_{uuid.uuid4().hex[:12]}", rule_id=rule.rule_id, strategy=rule.strategy, fingerprint=fingerprint, window_expires_at=window_expires)
        group.add_alert(alert)
        group.update_summary()
        await self.store.store_group(group)
        await self.store.store_fingerprint_mapping(fingerprint, group.group_id, expires_at=window_expires)
        should_notify = group.alert_count >= rule.max_alerts_before_notify
        if should_notify:
        ...
        for cb in self._on_group_created:
            try:
                ...
                logger.error("callback_error", error=str(e))
        logger.info("correlation_group_created", group_id=group.group_id, rule_id=rule.rule_id, should_notify=should_notify)
        return CorrelationResult(alert=alert, correlated=True, group=group, new_group=True, rule_matched=rule, should_notify=should_notify, suppression_reason=None if should_notify else "Waiting for more alerts")

    async def get_group(self, group_id: str) -> AlertGroup | None:
        return await self.store.get_group(group_id)

    async def get_active_groups(self, service: str | None = None, limit: int = 100) -> list[AlertGroup]:
        return await self.store.get_active_groups(service=service, limit=limit)

    async def close_group(self, group_id: str, status: AlertGroupStatus = AlertGroupStatus.CLOSED) -> bool:
    ...
    async def cleanup_stale_groups(self) -> int:
        return await self.store.cleanup_stale_groups(stale_after_seconds=getattr(self.settings, "correlation_stale_after_seconds", 3600))
