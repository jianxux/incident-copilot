...
class CorrelationStore:
    ...
    async def store_group(self, group: AlertGroup) -> None:
        ...
        if self._use_memory:
            ...
            if group.service:
                self._memory_store.setdefault(f"{KEY_SERVICE_GROUPS}{group.service}", set()).add(group.group_id)
        else:
        ...
        if self._use_memory:
            if key in self._memory_store:
                ...
                if group and group.service:
                    self._memory_store.get(f"{KEY_SERVICE_GROUPS}{group.service}", set()).discard(group_id)
                return True
            return False
    ...
    async def get_active_groups(self, service: str | None = None, limit: int = 100) -> list[AlertGroup]:
        if self._use_memory:
            group_ids = list(self._memory_store.get(f"{KEY_SERVICE_GROUPS}{service}" if service else KEY_ACTIVE_GROUPS, set()))[:limit]
        else:
            group_ids = list(await self._redis.smembers(f"{KEY_SERVICE_GROUPS}{service}" if service else KEY_ACTIVE_GROUPS))[:limit]
        groups = [g for gid in group_ids if (g := await self.get_group(gid)) and g.status == AlertGroupStatus.ACTIVE]
        groups.sort(key=lambda g: g.last_alert_at or g.created_at, reverse=True)
        return groups

    async def store_fingerprint_mapping(self, fingerprint: str, group_id: str, expires_at: datetime | None = None) -> None:
        key = f"{KEY_FINGERPRINT}{fingerprint}"
        if self._use_memory:
            self._memory_store[key] = {"group_id": group_id, "expires_at": expires_at.isoformat() if expires_at else None}
        else:
            ttl = max(1, int((expires_at - datetime.utcnow()).total_seconds())) if expires_at else self.group_ttl
    ...
    async def get_group_by_fingerprint(self, fingerprint: str) -> AlertGroup | None:
        ...
        if self._use_memory:
            ...
            if mapping:
                if mapping.get("expires_at") and datetime.utcnow() > datetime.fromisoformat(mapping["expires_at"]):
                    del self._memory_store[key]
                    return None
    ...
    async def get_all_rules(self, enabled_only: bool = True) -> list[CorrelationRule]:
        rule_ids = self._memory_store.get("correlation:rules", set()) if self._use_memory else await self._redis.smembers("correlation:rules")
        rules = [r for rid in rule_ids if (r := await self.get_rule(rid)) and (not enabled_only or r.enabled)]
    ...
    async def get_stats(self) -> dict[str, Any]:
        if self._use_memory:
            return {"mode": "memory", "active_groups": len(self._memory_store.get(KEY_ACTIVE_GROUPS, set())), "rules": len(self._memory_store.get("correlation:rules", set()))}
        return {"mode": "redis", "active_groups": await self._redis.scard(KEY_ACTIVE_GROUPS), "rules": await self._redis.scard("correlation:rules")}
