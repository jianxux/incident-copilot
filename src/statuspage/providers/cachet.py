...
class CachetProvider:
    ...
    async def get_components(self) -> list[Component]:
        ...
        return [
            Component(
                ...
                status=STATUS_REV.get(c.get("status", 1), ComponentStatus.OPERATIONAL),
                group_id=str(c.get("group_id", "")) if c.get("group_id") else None,
                position=c.get("order", 0),
            )
    ...
    async def create_incident(self, incident: StatusPageIncident) -> StatusPageIncident:
        payload = {
            ...
            "status": INC_STATUS.get(incident.status, 1),
            "visible": 1,
            "notify": True,
        }
    ...
    async def update_incident(
        ...
        await self.client.put(
            ...
            json={"status": INC_STATUS.get(update.status, 1)},
        )
        r = await self.client.get(f"{self.base_url}/incidents/{incident_id}")
        d = r.json().get("data", {})
        return StatusPageIncident(
            id=str(d.get("id", incident_id)),
            external_id=str(d.get("id", incident_id)),
            name=d.get("name", ""),
    ...
    async def create_maintenance(self, m: MaintenanceWindow) -> MaintenanceWindow:
        payload = {
            ...
            "status": 0,
            "scheduled_at": m.scheduled_start.isoformat(),
            "notify": m.notify_subscribers,
        }
        if m.component_ids:
