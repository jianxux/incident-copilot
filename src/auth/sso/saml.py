...
<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
    ...
    def _encode_request(self, xml_request: str, use_deflate: bool = True) -> str:
        ...
        if use_deflate:
            ...
            compressed = zlib.compress(xml_request.encode("utf-8"))[2:-4]  # Strip zlib header/checksum
    ...
    def _map_attributes_to_user(
        ...
        if not email:
            for attr_name in ["email", "Email", "emailAddress", "mail", "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"]:
    ...
    def generate_metadata(
        ...
        """
        want_signed = want_assertions_signed if want_assertions_signed is not None else self.settings.want_assertions_signed
        want_msg_signed = want_messages_signed if want_messages_signed is not None else self.settings.want_messages_signed
