...
class CommandHandler:
    ...
    async def handle(self, ctx: CommandContext) -> dict[str, Any]:
        ...
        handlers = {"status": self._handle_status, "search": self._handle_search, "recent": self._handle_recent, "runbook": self._handle_runbook, "help": self._handle_help}
    ...
    async def _handle_status(self, ctx, args):
        ...
        incident_dicts = [{"incident_id": inc.incident_id, "title": inc.title, "service_name": inc.service_name, "severity": inc.severity.value, "status": inc.status, "triggered_at": inc.triggered_at} for inc in incidents]
        return BlockKitBuilder.status_response(incident_dicts, stats)

    async def _handle_search(self, ctx, args):
        query = args.strip()
        if not query:
            return BlockKitBuilder.error_response("Search query required", "Usage: /incident search <query>")
        incidents = await incident_store.get_all_incidents()
        results = [{"incident_id": inc.incident_id, "title": inc.title, "service_name": inc.service_name, "severity": inc.severity.value} for inc in incidents if query.lower() in inc.title.lower() or query.lower() in inc.service_name.lower()]
    ...
    async def _handle_recent(self, ctx, args):
        ...
        incidents = await incident_store.get_all_incidents()
        incident_dicts = [{"incident_id": inc.incident_id, "title": inc.title, "service_name": inc.service_name, "severity": inc.severity.value, "status": inc.status, "triggered_at": inc.triggered_at} for inc in incidents[:count]]
        return BlockKitBuilder.recent_response(incident_dicts, count)

    async def _handle_runbook(self, ctx, args):
        service = args.strip()
        if not service:
            return BlockKitBuilder.error_response("Service name required", "Usage: /incident runbook <service>")
        try:
            matches = self.runbook_linker.find_relevant_runbooks(query=service, service_name=service, top_k=5, min_score=0.05)
            runbook_dicts = [{"title": rb.title, "url": rb.url, "source": rb.source, "relevance_score": rb.relevance_score, "matched_terms": rb.matched_terms} for rb in matches]
        except:
            runbook_dicts = []
