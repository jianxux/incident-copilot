...
class TimelineBuilder:
    ...
    def build_from_context_card(self, card: ContextCard, incident_data: dict) -> list[TimelineEvent]:
        ...
        if card.github and card.github.recent_deployments:
            for deploy in card.github.recent_deployments:
                if deploy.deployed_at and deploy.deployed_at < (card.triggered_at or datetime.now(timezone.utc)):
                    self.add_event(
        ...
        if incident_data.get("notification_sent"):
            self.add_event(
                timestamp=incident_data.get("notification_time", datetime.now(timezone.utc)),
