...
class MaintenanceService:
    ...
    async def _notify(
        self, window: MaintenanceWindow, ntype: NotificationType, message: str
    ) -> None:
    ...
    async def create_window(
        self, request: MaintenanceWindowCreate, created_by: str
    ) -> MaintenanceWindow:
    ...
    async def get_upcoming_windows(self, hours: int = 24) -> list[MaintenanceWindow]:
        ...
        return sorted(
            [
                ...
                if w.status in (MaintenanceStatus.SCHEDULED,)
                and now <= w.schedule.start_time <= future
            ],
    ...
    async def approve(
        ...
        window.approvals.append(
            ApprovalRecord(approver_id=approver_id, approved=True, comment=comment)
        )
    ...
    async def extend_maintenance(
        self, window_id: UUID, request: ExtendMaintenanceRequest
    ) -> MaintenanceWindow:
        window = self._windows.get(window_id)
        ...
        await self._notify(
            ...
            f"Extended {request.extend_minutes}min: {request.reason}",
        )
        return window
    ...
    async def detect_overlaps(self, window: MaintenanceWindow) -> list[OverlapWarning]:
        ...
        for other in self._windows.values():
            if other.id == window.id or other.status in (
                MaintenanceStatus.CANCELLED,
                MaintenanceStatus.COMPLETED,
            ):
                continue
            if not (
                window.schedule.start_time < other.schedule.end_time
                and window.schedule.end_time > other.schedule.start_time
            ):
                continue
    ...
    async def process_scheduled_windows(self) -> list[MaintenanceWindow]:
        ...
        for window in self._windows.values():
            if (
                ...
                and window.schedule.start_time <= now
                and window.is_approved()
            ):
