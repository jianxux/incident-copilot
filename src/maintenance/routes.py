...
from .models import (MaintenanceWindow, MaintenanceWindowCreate, MaintenanceWindowUpdate,
    MaintenanceStatus, ScopeType, ExtendMaintenanceRequest, OverlapWarning)
...
async def create_window(request: MaintenanceWindowCreate, created_by: str = Query(...), svc: MaintenanceService = Depends(get_svc)):
    try:
...
async def list_windows(status: Optional[MaintenanceStatus] = None, scope_type: Optional[ScopeType] = None,
    from_time: Optional[datetime] = None, to_time: Optional[datetime] = None, 
    limit: int = Query(50, ge=1, le=200), svc: MaintenanceService = Depends(get_svc)):
...
async def get_active(at_time: Optional[datetime] = None, svc: MaintenanceService = Depends(get_svc)):
    return await svc.get_active_windows(at_time)


@router.get("/upcoming", response_model=UpcomingResponse)
async def get_upcoming(hours: int = Query(24, ge=1, le=168), svc: MaintenanceService = Depends(get_svc)):
    windows = await svc.get_upcoming_windows(hours)
    return UpcomingResponse(windows=windows, next_maintenance=windows[0].schedule.start_time if windows else None)
...
async def update_window(window_id: UUID, request: MaintenanceWindowUpdate, svc: MaintenanceService = Depends(get_svc)):
    try:
...
async def check_status(scope_type: ScopeType, identifier: str, at_time: Optional[datetime] = None, svc: MaintenanceService = Depends(get_svc)):
    in_maint, window = await svc.is_in_maintenance(scope_type, identifier, at_time)
    msg = f"{identifier} in maintenance: {window.title}" if in_maint and window else f"{identifier} not in maintenance"
    return MaintenanceCheckResponse(in_maintenance=in_maint, window=window, message=msg)


@router.post("/check/alerts")
async def check_alerts(alerts: list[dict], at_time: Optional[datetime] = None, svc: MaintenanceService = Depends(get_svc)):
    return await svc.suppress_alerts(alerts, at_time)


@router.post("/{window_id}/approve", response_model=MaintenanceWindow)
async def approve(window_id: UUID, req: ApprovalRequest, svc: MaintenanceService = Depends(get_svc)):
    try:
...
async def extend(window_id: UUID, req: ExtendMaintenanceRequest, svc: MaintenanceService = Depends(get_svc)):
    try:
...
async def export_calendar(from_time: Optional[datetime] = None, to_time: Optional[datetime] = None,
    scope_type: Optional[ScopeType] = None, svc: MaintenanceService = Depends(get_svc)):
    windows = await svc.list_windows(from_time=from_time or datetime.utcnow(), 
        to_time=to_time or datetime.utcnow() + timedelta(days=30), scope_type=scope_type)
    return Response(MaintenanceScheduler().generate_ical_calendar(windows), 
        media_type="text/calendar", headers={"Content-Disposition": "attachment; filename=maintenance.ics"})
...
async def export_window_ical(window_id: UUID, svc: MaintenanceService = Depends(get_svc)):
    ...
    if not window:
        raise HTTPException(404, "Not found")
    return Response(MaintenanceScheduler().generate_ical_calendar([window]),
        media_type="text/calendar", headers={"Content-Disposition": f"attachment; filename=maintenance-{window_id}.ics"})


@router.post("/annotate-incident")
async def annotate(incident_id: str = Query(...), scope_type: ScopeType = Query(...), 
    identifier: str = Query(...), svc: MaintenanceService = Depends(get_svc)):
    annotation = await svc.annotate_incident(incident_id, scope_type, identifier)
    return {"annotated": bool(annotation), "annotation": annotation} if annotation else {"annotated": False}
