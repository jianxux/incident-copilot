...
class MaintenanceScheduler:
    ...
    def get_next_occurrences(
        ...
        until = (
            datetime.strptime(parts["UNTIL"][:8], "%Y%m%d")
            if "UNTIL" in parts
            else schedule.recurrence_end
        )
        ...
        while len(occurrences) < count and inst < count_limit:
            ...
            if self._match_byday(current, byday) and current >= from_time:
                occurrences.append(
                    RecurrenceInstance(UUID(int=0), current, current + duration, inst)
                )
    ...
    def generate_ical_calendar(self, windows: list[MaintenanceWindow]) -> str:
        header = [
            "BEGIN:VCALENDAR",
            "VERSION:2.0",
            "PRODID:-//Incident Copilot//Maintenance//EN",
            "CALSCALE:GREGORIAN",
            "METHOD:PUBLISH",
            "X-WR-CALNAME:Maintenance Windows",
        ]
        return "\r\n".join(
            header + [self.generate_ical_event(w) for w in windows] + ["END:VCALENDAR"]
        )
    ...
    def next_maintenance_for(
        ...
        for w in windows:
            ...
            start = (
                ...
                if w.schedule.is_recurring
                else w.schedule.start_time
            )
            if start < now:
                continue
