...
async def create_webhook(config: WebhookConfigCreate, org_id: OrgId) -> WebhookConfig:
    """Create a new webhook configuration."""
    webhook = WebhookConfig(
        **config.model_dump(), organization_id=org_id, secret=generate_signing_secret()
    )
...
async def update_webhook(
    webhook_id: UUID, update: WebhookConfigUpdate, org_id: OrgId
) -> WebhookConfig:
...
async def test_webhook(
    webhook_id: UUID, request: WebhookTestRequest, org_id: OrgId
) -> WebhookTestResponse:
...
async def list_deliveries(
    webhook_id: UUID, org_id: OrgId, limit: int = Query(default=50, le=200)
...
async def bulk_deliver(
    ...
    for event in request.events:
        if event.organization_id != org_id:
            raise HTTPException(status_code=403, detail="Event organization mismatch")
    ...
    deliveries = await webhook_service.deliver_bulk(request.events, request.webhook_ids)
