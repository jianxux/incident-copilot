...
from ..models import (
    ChangeSource, ChangeStatus, FeatureFlag, RiskLevel
)
...
class LaunchDarklyCollector:
    ...
    async def _get_audit_log(
        ...
        flag_actions = {
            "updateOn", "updateOff", "updateFlagVariations",
            "updateRules", "updateFallthrough", "updateTargets",
            "createFlag", "deleteFlag", "updateFlagDefaultVariations"
        }
    ...
    async def get_flag_state(self, flag_key: str, environment: str = "production") -> Optional[dict]:
        """Get current state of a feature flag."""
        client = await self._get_client()
        
        resp = await client.get(
            f"/flags/{self.project_key}/{flag_key}",
