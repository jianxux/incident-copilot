...
from ..models import (
    ChangeSource, ChangeStatus, Deployment, RiskLevel
)
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._client is None:
            self._client = httpx.AsyncClient(
                ...
                timeout=30.0,
                verify=False  # ArgoCD often uses self-signed certs
            )
        return self._client
        ...
        for idx, entry in enumerate(history):
            ...
            if idx > 0:
                ...
                if entry.get("id", 0) > prev.get("id", 0):
                    is_rollback = entry.get("revision") in [h.get("revision") for h in history[:idx-1]]
