...
from .models import (
    ChangeEvent, ChangeType, ChangeSource, ChangeStatus, RiskLevel,
    Deployment, ConfigChange, FeatureFlag, ChangeFreeze,
    ChangeCorrelation, ChangeTimeline
)
...
class ChangeTrackingService:
    ...
    async def correlate_changes(
        ...
        if service:
            changes = [
                c for c in changes
                if service in ([c.service] + c.services)
            ]
    ...
    async def calculate_impact_score(self, change: ChangeEvent) -> float:
        ...
        # Service count
        total_services = len(set([change.service] + change.services)) if change.service else len(change.services)
        if total_services > 5:
            score += 0.2
