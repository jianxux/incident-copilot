...
from ..models import (
    ...
    Rotation, RotationType, ProviderType, ScheduleSyncResult, OverrideStatus
)
        ...
        for rotation in timeline.get("finalTimeline", {}).get("rotations", []):
            for period in rotation.get("periods", []):
                ...
                shifts.append(OnCallShift(
                    ...
                    start_time=datetime.fromisoformat(period["startDate"].replace("Z", "+00:00")),
    ...
    async def get_rotations(self, schedule_id: str) -> list[Rotation]:
        ...
        for idx, rot in enumerate(schedule_data.get("rotations", [])):
            ...
            for p in rot.get("participants", []):
                participants.append(OnCallUser(
            ...
            start_date = datetime.fromisoformat(rot["startDate"].replace("Z", "+00:00")) if rot.get("startDate") else datetime.utcnow()
