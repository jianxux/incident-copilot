...
from .models import (
    ...
    Rotation, ProviderType, OverrideStatus, HandoffNotification,
    OnCallHistoryEntry, ScheduleSyncResult
)
    ...
    async def sync_all_schedules(self) -> list[ScheduleSyncResult]:
        ...
        if self._pagerduty:
            try:
                ...
                results.append(ScheduleSyncResult(
    ...
    async def who_is_oncall(
        ...
        for override in self._overrides.values():
            if (override.schedule_id == schedule_id and 
                override.status == OverrideStatus.ACTIVE and
                override.start_time <= check_time <= override.end_time):
