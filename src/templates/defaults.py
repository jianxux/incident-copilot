...
from .models import (
    IncidentTemplate, InitialAction, MatchPattern, StakeholderRole,
    TemplateCategory, TemplateField, FieldType,
)

def _t(id: str, name: str, desc: str, cat: TemplateCategory, title: str, sev: str,
       runbooks: list[str], actions: list[InitialAction], stakeholders: list[StakeholderRole],
       fields: list[TemplateField], patterns: list[MatchPattern], tags: list[str]) -> IncidentTemplate:
    """Helper to create a built-in template."""
    return IncidentTemplate(
        id=id, name=name, description=desc, category=cat, title_pattern=title,
        severity_default=sev, runbook_urls=runbooks, initial_actions=actions,
        stakeholders=stakeholders, fields=fields, match_patterns=patterns,
        is_builtin=True, tags=tags,
    )

DATABASE_OUTAGE = _t(
    "builtin-database-outage", "Database Outage", "Database connectivity or performance issues",
    TemplateCategory.DATABASE, "[DB] {service} - {issue_type}", "critical",
    ...
    [InitialAction(order=1, title="Check database cluster health", estimated_minutes=2),
     ...
     InitialAction(order=4, title="Initiate failover if primary down", assignee_role="dba_oncall")],
    [StakeholderRole(role="dba_oncall", notification_channel="pagerduty", required=True),
     StakeholderRole(role="platform_lead", notification_channel="slack", escalation_delay_minutes=15)],
    [TemplateField(name="database_name", label="Database Name", required=True),
     TemplateField(name="issue_type", label="Issue Type", field_type=FieldType.SELECT,
...
DEPLOYMENT_FAILURE = _t(
    "builtin-deployment-failure", "Deployment Failure", "Failed or problematic deployments",
    TemplateCategory.DEPLOYMENT, "[Deploy] {service} deployment {status}", "high",
    ...
    [InitialAction(order=1, title="Identify failing deployment", estimated_minutes=2),
     ...
     InitialAction(order=4, title="Execute rollback if needed", assignee_role="oncall", estimated_minutes=10)],
    ...
    [TemplateField(name="service", label="Service", field_type=FieldType.SERVICE, required=True),
     TemplateField(name="status", label="Status", field_type=FieldType.SELECT,
...
SECURITY_INCIDENT = _t(
    "builtin-security-incident", "Security Incident", "Security-related incidents requiring immediate response",
    TemplateCategory.SECURITY, "[SECURITY] {incident_type} - {severity_level}", "critical",
    ...
    [InitialAction(order=1, title="Assess scope and impact", estimated_minutes=10),
     ...
     InitialAction(order=4, title="Notify security team lead", assignee_role="incident_commander")],
    ...
    [TemplateField(name="incident_type", label="Incident Type", field_type=FieldType.SELECT, required=True,
                  options=["Data Breach", "Unauthorized Access", "Malware", "DDoS", "Phishing"]),
     TemplateField(name="severity_level", label="Security Severity", field_type=FieldType.SELECT,
...
API_OUTAGE = _t(
    "builtin-api-outage", "API/Service Outage", "API or microservice outages",
    TemplateCategory.APPLICATION, "[Outage] {service} - {symptom}", "high",
    ...
    [StakeholderRole(role="oncall", notification_channel="pagerduty", required=True),
     StakeholderRole(role="service_owner", notification_channel="slack", escalation_delay_minutes=10)],
    [TemplateField(name="service", label="Service", field_type=FieldType.SERVICE, required=True),
     TemplateField(name="symptom", label="Symptom", field_type=FieldType.SELECT,
...
NETWORK_ISSUE = _t(
    "builtin-network-issue", "Network Issue", "Network connectivity and routing problems",
    TemplateCategory.NETWORK, "[Network] {issue_type} - {affected_region}", "high",
    ...
    [TemplateField(name="issue_type", label="Issue Type", field_type=FieldType.SELECT,
     ...
     TemplateField(name="affected_region", label="Affected Region", field_type=FieldType.SELECT,
...
THIRD_PARTY_OUTAGE = _t(
    "builtin-third-party-outage", "Third-Party Service Outage", "External vendor/service outages",
    TemplateCategory.THIRD_PARTY, "[Vendor] {vendor_name} - {impact}", "medium",
    ...
    [TemplateField(name="vendor_name", label="Vendor Name", required=True),
     TemplateField(name="impact", label="Impact", field_type=FieldType.SELECT,
...
INFRASTRUCTURE_SCALING = _t(
    "builtin-infrastructure-scaling", "Infrastructure Scaling Issue", "Capacity and scaling problems",
    TemplateCategory.INFRASTRUCTURE, "[Capacity] {resource_type} exhaustion - {service}", "high",
    ...
    [TemplateField(name="resource_type", label="Resource Type", field_type=FieldType.SELECT, required=True,
...
BUILTIN_TEMPLATES: list[IncidentTemplate] = [
    DATABASE_OUTAGE, DEPLOYMENT_FAILURE, SECURITY_INCIDENT, API_OUTAGE,
    NETWORK_ISSUE, THIRD_PARTY_OUTAGE, INFRASTRUCTURE_SCALING,
]
