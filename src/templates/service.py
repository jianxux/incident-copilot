...
from .models import (
    AppliedTemplate, IncidentTemplate, TemplateAnalytics, TemplateCreateRequest,
    TemplateExport, TemplateMatch, TemplateUpdateRequest, TemplateVersion,
)
...
class TemplateService:
    ...
    async def get_all(self, org_id: str | None = None, include_inactive: bool = False) -> list[IncidentTemplate]:
    ...
    async def create(self, req: TemplateCreateRequest, org_id: str | None = None, created_by: str | None = None) -> IncidentTemplate:
        """Create a new template."""
        now = datetime.utcnow()
        template = IncidentTemplate(
            id=f"template-{uuid.uuid4().hex[:12]}", name=req.name, description=req.description,
            category=req.category, title_pattern=req.title_pattern, severity_default=req.severity_default,
            runbook_urls=req.runbook_urls, initial_actions=req.initial_actions,
            stakeholders=req.stakeholders, fields=req.fields, match_patterns=req.match_patterns,
            tags=req.tags, organization_id=org_id, created_at=now, updated_at=now, created_by=created_by,
        )
        if org_id:
    ...
    async def update(self, template_id: str, req: TemplateUpdateRequest, org_id: str | None = None, updated_by: str | None = None) -> IncidentTemplate | None:
        ...
        # Save version
        template.version_history.append(TemplateVersion(
            version=template.version, created_at=template.updated_at, created_by=template.created_by,
    ...
    async def suggest(self, title: str, description: str | None = None, service: str | None = None,
                      source: str | None = None, tags: list[str] | None = None,
                      org_id: str | None = None, limit: int = 5) -> list[TemplateMatch]:
        """Suggest templates based on alert content."""
        return suggest_templates(await self.get_all(org_id), title, description, service, source, tags, limit)

    async def apply(self, template_id: str, field_values: dict[str, Any], org_id: str | None = None) -> AppliedTemplate | None:
        ...
        return AppliedTemplate(
            template_id=template.id, template_name=template.name, generated_title=title,
            severity=template.severity_default, runbook_urls=template.runbook_urls,
            initial_actions=template.initial_actions, stakeholders=template.stakeholders,
            custom_fields=field_values,
        )

    async def customize(self, template_id: str, org_id: str, created_by: str | None = None) -> IncidentTemplate | None:
    ...
    async def record_resolution(self, template_id: str, resolution_minutes: float, escalated: bool, org_id: str | None = None) -> None:
        ...
        if a.avg_resolution_time_minutes is None:
            ...
            a.avg_resolution_time_minutes = (a.avg_resolution_time_minutes * (count - 1) + resolution_minutes) / count
        
        success = 0.0 if escalated else 1.0
        a.success_rate = success if a.success_rate is None else (a.success_rate * (count - 1) + success) / count
        self._save(template)

    async def get_analytics(self, org_id: str | None = None) -> list[dict]:
        """Get analytics for all templates."""
        return [{"template_id": t.id, "template_name": t.name, "category": t.category,
                 "is_builtin": t.is_builtin, **t.analytics.model_dump()}
                for t in await self.get_all(org_id, include_inactive=True)]

    async def export_templates(self, template_ids: list[str] | None = None, org_id: str | None = None) -> TemplateExport:
    ...
    async def import_templates(self, export: TemplateExport, org_id: str, created_by: str | None = None) -> list[IncidentTemplate]:
