...
WORKLOAD_BENCHMARKS = {
    "gini_coefficient": {"elite": 0.15, "high": 0.25, "medium": 0.35},
    "top_10_share": {"elite": 15, "high": 25, "medium": 35},
}


def classify_metric(metric_name: str, value: float) -> PerformanceTier:
    return (
        DORA_BENCHMARKS[metric_name].classify(value)
        if metric_name in DORA_BENCHMARKS
...
def classify_team(metrics: TeamMetrics) -> dict[str, PerformanceTier]:
    return {
        ...
        if (v := getattr(metrics, b.metric, None)) is not None
    }
...
def calculate_overall_tier(classifications: dict[str, PerformanceTier]) -> PerformanceTier:
    ...
    return (
        ...
        if avg >= 3.5
            ...
            if avg >= 2.5
            else (PerformanceTier.MEDIUM if avg >= 1.5 else PerformanceTier.LOW)
        )
    )
...
def compare_to_industry(metrics: TeamMetrics) -> dict:
    ...
    descs = {
        PerformanceTier.ELITE: "Top 10% of teams",
        PerformanceTier.HIGH: "Above average, top 30%",
        PerformanceTier.MEDIUM: "Average, room for improvement",
        PerformanceTier.LOW: "Below average, needs work",
    }
    return {
