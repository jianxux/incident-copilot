...
def calculate_engineer_workloads(
    ...
    for i in incidents:
        if period.start <= i.created_at <= period.end and (
            assignee := getattr(i, "assignee_id", None)
        ):
            workloads[assignee] = workloads.get(assignee, 0) + severity_weights.get(
                getattr(i, "severity", "medium"), 2
            )
    return workloads
...
def calculate_workload_distribution(
    ...
    return WorkloadDistribution.from_workloads(
        list(calculate_engineer_workloads(incidents, period).values())
    )


def calculate_oncall_burden(
    oncall_shifts: list[dict], engineer_id: str, period: PerformancePeriod
) -> float:
    total = 0.0
    for s in oncall_shifts:
        if s.get("engineer_id") != engineer_id:
            continue
...
def calculate_after_hours_incidents(
    ...
    biz_hours: tuple[int, int] = (9, 18),
) -> int:
    count = 0
    for i in incidents:
        if (
            period.start <= i.created_at <= period.end
            and getattr(i, "assignee_id", None) == engineer_id
        ):
            if (
                i.created_at.hour < biz_hours[0]
                or i.created_at.hour >= biz_hours[1]
                or i.created_at.weekday() >= 5
            ):
                count += 1
    return count
...
def calculate_burnout_risk(
    ...
    if consecutive > 7:
        score, factors = score + 30, factors + [f"Extended oncall: {consecutive} days"]
    elif consecutive > 4:
        score, factors = score + 15, factors + [f"Long oncall: {consecutive} days"]

    oncall_hrs = calculate_oncall_burden(oncall_shifts, engineer_id, period)
    if oncall_hrs > period.days * 24 / 5 * 1.5:
        score, factors = score + 20, factors + [f"Excessive oncall: {oncall_hrs:.0f}h"]
    ...
    if avg_dur > 4:
        score, factors = score + 10, factors + [f"Long avg resolution: {avg_dur:.1f}h"]
    ...
    risk = (
        ...
        if score >= 60
            ...
            BurnoutRisk.HIGH
            if score >= 40
            else (BurnoutRisk.MODERATE if score >= 20 else BurnoutRisk.LOW)
        )
    )
    ...
    return BurnoutIndicators(
        ...
        avg_incident_duration_hours=round(avg_dur, 1),
...
def _calc_consecutive_oncall(shifts: list[dict], eng_id: str, ref: datetime) -> int:
    eng_shifts = sorted(
        [s for s in shifts if s.get("engineer_id") == eng_id],
...
def _burnout_recs(factors: list[str], risk: BurnoutRisk) -> list[str]:
    ...
    if risk in (BurnoutRisk.HIGH, BurnoutRisk.CRITICAL):
        recs += [
            "Consider immediate oncall rotation adjustment",
            "Schedule 1:1 to discuss workload",
        ]
    if any("after-hours" in f.lower() for f in factors):
        recs.append("Review paging thresholds")
    if any("oncall" in f.lower() for f in factors):
        recs.append("Ensure break between rotations")
