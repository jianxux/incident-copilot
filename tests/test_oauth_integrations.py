...
@pytest.mark.unit
def test_reconnect_updates_connected_at():
    """On reconnect, created_at and status connected_at should reflect the new connection time."""
    import time
    ...
    # created_at should be updated (not preserved from first connection)
    assert token_v2.created_at > first_created
    assert token_v2.access_token == "token-v2"

    # Status endpoint should return connected_at
    response = client.get("/api/integrations/pagerduty/status", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["connected"] is True
    assert data["connected_at"] is not None
    assert data["connected_at"] >= token_v2.updated_at.isoformat()
