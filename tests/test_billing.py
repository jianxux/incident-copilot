...
def _make_settings(**overrides):
    ...
    s.stripe_publishable_key = overrides.get(
        "stripe_publishable_key", "pk_test_placeholder123"
    )
    s.stripe_webhook_secret = overrides.get(
        "stripe_webhook_secret", "whsec_test_placeholder123"
    )
    ...
    s.stripe_price_enterprise = overrides.get(
...
def mock_stripe():
    ...
    mock = MagicMock()
    with (
        patch("src.billing.service.stripe", mock),
...
class TestBillingService:
    ...
    def test_is_not_configured_without_keys(self):
        with patch(
            "src.billing.service.get_settings",
            return_value=_make_settings(stripe_api_key="", stripe_publishable_key=""),
        ):
            svc = BillingService()
...
class TestCreateCustomer:
    ...
    async def test_create_customer_success(self, service, mock_stripe):
        ...
        customer_id = await service.create_customer(
            tenant, "owner@acme.com", "Acme Corp"
        )
    ...
    async def test_create_customer_not_configured(self):
        with patch(
            "src.billing.service.get_settings",
            return_value=_make_settings(stripe_api_key=""),
        ):
            svc = BillingService()
...
class TestCreatePortalSession:
    ...
    async def test_portal_success(self, service, mock_stripe):
        ...
        url = await service.create_portal_session(
            tenant, return_url="http://localhost/billing"
        )
...
class TestWebhookHandlers:
    ...
    async def test_handle_checkout_completed(self, service, mock_stripe):
        ...
        with patch("src.billing.service.auth_service") as mock_auth:
            ...
            mock_auth.update_tenant_plan.assert_called_once_with(
                "tenant_1", PlanTier.PRO
            )
...
class TestWebhookEndpoint:
    ...
    async def test_webhook_valid_signature(self, mock_stripe, mock_settings):
        ...
        with (
            patch("src.billing.routes.billing_service") as mock_svc,
...
class TestIntegrationSlotMiddleware:
    ...
    async def test_slot_exceeded(self):
        ...
        tenant = _make_tenant(
            plan=PlanTier.FREE, integrations={"slack": {"token": "x"}}
        )
        auth = AuthContext(user=_make_user(), tenant=tenant)
