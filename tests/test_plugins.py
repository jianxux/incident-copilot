...
import pytest
from src.plugins import PayloadTransformer, PluginEvent, PluginRegistry, PluginStatus, PluginType, WebhookConfig, WebhookExecutor
from src.plugins.models import EnrichmentConfig, FilterCondition, FilterConfig, HmacConfig, PluginCreateRequest, PluginUpdateRequest, RetryConfig
from src.plugins.registry import PluginExistsError, PluginNotFoundError

@pytest.fixture
def sample_context_card():
    return {"incident_id": "INC-12345", "title": "High error rate on payments-api", "severity": "high", "service_name": "payments-api", "owners": ["team"], "ai_summary": {"explanation": "Test"}, "runbooks": []}

@pytest.fixture
def webhook_plugin_request():
    return PluginCreateRequest(id="test-webhook", name="Test", type=PluginType.WEBHOOK, webhook_config=WebhookConfig(url="https://example.com"))
...
class TestPluginRegistry:
    ...
    async def test_register_webhook_without_config(self, registry):
        with pytest.raises(ValueError, match="webhook_config"):
            await registry.register(PluginCreateRequest(id="bad-webhook", name="Bad", type=PluginType.WEBHOOK))
    ...
    async def test_update_plugin(self, registry, webhook_plugin_request):
        await registry.register(webhook_plugin_request)
        plugin = await registry.update("test-webhook", PluginUpdateRequest(name="Updated", status=PluginStatus.DISABLED))
...
class TestWebhookExecutor:
    ...
    async def test_send_webhook_success(self):
        ...
        with patch.object(executor, "_attempt_delivery", new_callable=AsyncMock) as mock:
            ...
            delivery = await executor.send(url="https://example.com", method="POST", payload={}, plugin_id="test")
...
class TestPayloadTransformer:
    ...
    def test_transform_with_filters(self, sample_context_card):
        result = PayloadTransformer().transform('{"title": "{{ title | truncate(20) }}"}', sample_context_card)
    ...
    def test_render_string(self, sample_context_card):
        assert PayloadTransformer().render_string("ID: {{ incident_id }}", sample_context_card) == "ID: INC-12345"

class TestFilterPlugin:
    @pytest.mark.asyncio
    async def test_filter_include(self, registry, sample_context_card):
        await registry.register(PluginCreateRequest(id="filter-test", name="Filter", type=PluginType.FILTER, filter_config=FilterConfig(conditions=[FilterCondition(field="severity", operator="eq", value="high")])))
    ...
    async def test_filter_modify(self, registry, sample_context_card):
        await registry.register(PluginCreateRequest(id="filter-mod", name="Modify", type=PluginType.FILTER, filter_config=FilterConfig(conditions=[FilterCondition(field="severity", operator="eq", value="high")], action="modify", modifications={"priority": "P1"})))
        result = await registry.test_plugin("filter-mod", sample_context_card, dry_run=False)
        assert result.details["action_taken"] == "modified" and result.details["result_data"]["priority"] == "P1"

class TestEnrichmentPlugin:
    @pytest.mark.asyncio
    async def test_enrichment_config(self, registry):
        plugin = await registry.register(PluginCreateRequest(id="enrich-test", name="Enrich", type=PluginType.ENRICHMENT, enrichment_config=EnrichmentConfig(url="https://api.example.com", target_field="metadata")))
...
class TestModelValidation:
    def test_plugin_id_validation(self):
        PluginCreateRequest(id="my-plugin", name="Test", type=PluginType.FILTER, filter_config=FilterConfig())
        with pytest.raises(ValueError):
            PluginCreateRequest(id="ab", name="Test", type=PluginType.FILTER, filter_config=FilterConfig())
